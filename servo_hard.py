...
# Raspberry Pi-based servo control
# Uses hardware PWM pins 12 and 13 (BCM) to control the servos.
# Since the Pi uses 3.3V outputs, this program is designed to work with simple MOSFET-based
# inverting level shifters, and thus the PWM output is inverted.
# Also, since the Pi uses the hardware PWMs to control output to the 3.5mm audio jack
# (which isn't on the Zero), it should not be used at the same time as this program.
# This program uses the pigpio library, which requires the pigpio daemon to be started.
import pigpio
import atexit
import json
